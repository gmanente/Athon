...
{
    public partial class RelatorioEmprestimosAtraso : CommonPage
    {
        protected new void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                try
                {
                    ...
                    if (Audit.Autenticar("RF019"))
                    {
                        report.Load(Server.MapPath(@"~\View\Report\Biblioteca\Stimull\RelatorioEmprestimosAtraso" + (ModeloRelatorio.Equals("2") ? "-Bruto.mrt" : ".mrt")));

                        StiReportOptions.SetAbsolutePath(GetSessao().IdCampus);
