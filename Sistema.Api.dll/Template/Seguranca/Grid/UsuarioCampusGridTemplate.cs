...
{
    public class UsuarioCampusGridTemplate : SubmoduloWireFrameTemplate
    {
        ...
        private long IdUsuarioCampus { get; set; }
        private long IdUsuario { get; set; }
        private long IdUsuarioParam { get; set; }
        public Div BoxInformacoesUsuario { get; set; }
        //Constructor
        public UsuarioCampusGridTemplate(string urlSubModulo, long idUsuario, long idCampus, long idUsuarioParam, bool acessoExterno)
            : base()
        {
            IdUsuarioCampus = 0;
            IdUsuario = idUsuario;
        ...
        public BtnDropDown GetFuncoesGrid()
        {
            ...
            if (Autenticar("RF014"))
            {
                ...
                {
                    Text = "Vincular Perfil",
                    Icone = "table",
                    Titulo = "Vincular Perfil",
                    Class = "item-acao-Perfil",
                    Url = "../Page/UsuarioPerfil.aspx",
                    Target = Target.Self,
                    JsInjection = new JsInjector("", @"// Funcionalidades
                                                    $('.item-acao-Perfil').click(function (e) {
                                                        e.preventDefault();
                                                        var href = $(this).attr('href') + '?idUsuario=" + IdUsuarioParam + @"&idUsuarioCampus=';
                                                        var idUsuarioCampus = $(this).parent('li').attr('data-id');
                                                        window.location = href + idUsuarioCampus;
                                                    });", "").Create()
                };
        ...
        public string GetAjaxCall()
        {
            ...
            {
                ContentCode = "var idUsuarioCampus = $(this).parent('li').attr('data-id');",
        ...
        public string MontarModalExcluir(long IdUsuarioCampus)
        {

            ModalExcluir.Id = "modal-excluir";
            ModalExcluir.Titulo = "Excluir Campus do usuario";
            ModalExcluir.Descricao = "Selecione o botão confirmar para realizar a exclusão do Campus do usuario.";

            var modalContentText = new P()
            {
                Text = "Caso confirme a exclusão do Campus, o mesmo será totalmente removido do sistema."
            };
