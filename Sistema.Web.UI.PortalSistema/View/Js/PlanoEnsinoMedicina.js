...
$(document).ready(function ()
{
    ...
    $("#lblNomeModulo").html('<li><i class="fa fa-home"></i><a href="/View/Page/PlanoEnsinoMedicina.aspx"> Plano de Ensino</a></li>')
...
function fnSalvarPlanoEnsino(idDisciplinaOfertaProfessor, idDisciplinaOferta, idPlanoEnsinoTopico, conteudoAbaSelecionada, idCampo)
{
    ...
    if (conteudoAbaSelecionada.trim() != '')
    {
        if ((conteudoAbaSelecionada.trim() != conteudoAnterior.trim()))
        {
            ...
            jqxhr = $.ajax({
                    type: 'POST',
                    url: '../Page/PlanoEnsinoMedicina.aspx/SalvarPlanoEnsino',
                    data: data,
...
function fnSalvarSemanaCronograma(semana, idCampoConteudo, idCampoMetodologia, idCampoRecurso) 
{
    ...
    else
    {
        if ((cronoConteudo != cronoConteudoAnterior) || (cronoMetodologia != cronoMetodologiaAnterior) || (cronoRecurso != cronoRecursoAnterior))
        {
            ...
            jqxhr = $.ajax({
                    type: 'POST',
                    url: '../Page/PlanoEnsinoMedicina.aspx/SalvarPlanoEnsinoCronograma',
                    data: data,
                    contentType: 'application/json; charset=utf-8',
                    dataType: 'json'
                })
...
function carregarPeriodoLetivo() {
    ...
    jqxhr = $.ajax({
        type: 'POST',
        url: '../Page/PlanoEnsinoMedicina.aspx/ListarPeriodoLetivo',
...
function ConsultarHorarios()
{
    ...
    $("#campusPediodoEscolhido2").text("Módulos: Campus - " + $('#Campus option:selected').text() + " > Período Letivo - " + $('#PeriodoLetivo option:selected').text());
    ...
    if (idCampus > 0 && idPeriodoLetivo > 0) {
        ...
        var jqxhr = $.ajax({
            type: 'POST',
            url: '../Page/PlanoEnsinoMedicina.aspx/ListarDisciplinas',
            data: '{ idCampus: "' + idCampus +
            ...
            if (!response.StatusOperacao) {
                ...
                if (listObj == null || listObj.length === 0) {
                    ...
                    $.each(listObj, function (key1, value1) {
                        ...
                        if (siglaCurso != 'MED')  //Listar Somente Medicina
                            return;
                    ...
                    else
                    {
                        $.each(lstDisciplinaOferta, function (key2, value2) {
                            ...
                            var dataInicioModulo = value2[0].GradeLetivaDisciplinaSemestre.DataInicio.substr(0,10);
                            var dataTerminoModulo = value2[0].GradeLetivaDisciplinaSemestre.DataTermino.substr(0,10);
                            ...
                            if ($("#authRf002").val() == "True") {
                                rf002 = '<li>' +
                                        ...
                                        '                              data-inicio="' + dataInicioModulo + '" ' +
                                        '                              data-termino="' + dataTerminoModulo + '" ' +
                            ...
                            if ($("#authRf003").val() == "True") {
                                rf003 = "<li>"
                                            + "<a href='../Report/PlanoEnsino/Aspx/PlanoEnsinoMedicinaRel.aspx?idDisciplinaOfertaProfessor=" + value2[0].DisciplinaOfertaHorarioProfessor.DisciplinaOfertaProfessor.Id + "&idGradeLetivaDisciplina=" + idGradeLetivaDisciplina + "&idDisciplinaOferta=" + value2[0].DisciplinaOferta.Id + "&idDisciplinaIntegrada=" + idDisciplinaIntegrada + "&idDisciplinaIntegradaTurma=" + idDisciplinaIntegradaTurma + "'"
                            ...
                            html += "<td style='text-align: center;'>" + setData(dataInicioModulo) + " à " + setData(dataTerminoModulo) + "</td>";
                            html += "<td>" + sigla + "</td>";
                            html += "<td>" + curso + "</td>";
                            //html += "<td>";
                        ...
                        $('.item-acao-registro').on("click", function (event) {
                            ...
                            var abaSelecionada = $("#hAbaSelecionada").val();
                            var conteudoAbaSelecionada = $("#hConteudoAbaSelecionada").val();
                            var areaSelecionada = $("#hAreaSelecionada").val();
                            var liSelecionado = $("#liSelecionado").val();
                            var mensagemTabSelecionada = $("#hMensagemtab").val();
                            var discipOfertaProfessor = $("#hidDisciplinaOfertaProfessor").val();
                            var discipOferta = $("#hidDisciplinaOferta").val();

                            if (abaSelecionada != "" && abaSelecionada != undefined) {
                                //$("#hAbaSelecionada").val("abaSelecionada");
                                //$("#hConteudoAbaSelecionada").val($("#areaEmenta").val());
                                //$("#liSelecionado").val("#lilinkEmenta");
                                //$("#hAreaSelecionada").val("#areaEmenta");
                                if ((discipOfertaProfessor) != ($(this).attr("data-idDisciplinaOfertaProfessor"))) {
                                    $("#hAbaSelecionada").val("tbEmenta");
                                    $("#hConteudoAbaSelecionada").val($("#areaEmenta").val());
                                    $("#liSelecionado").val("#lilinkEmenta");
                                    $("#hAreaSelecionada").val("#areaEmenta");
                            ...
                            $("#ModalTitulo").text("Plano de Ensino do Módulo - " + $(this).attr("data-disciplina"));
                            ...
                            var date1 = new Date($(this).attr("data-inicio"));
                            var date2 = new Date($(this).attr("data-termino"));
                            var date1Ms = date1.getTime();
                            var date2Ms = date2.getTime();
                            var differenceMs = Math.abs(date1Ms - date2Ms);
                            var totalSemanas = Math.floor(differenceMs / oneWeek) + 1;

                            $(".lilinkSemana").each(function () {
                                var id = $(this).attr('id');
                                var semana = id.substr(12);
                                if (semana > totalSemanas) {
                                    $(this).hide();
                                }
                            });

                            //====== FIM SEMANAS CRONOGRAMA =====//
                    ...
                    $('body').on('click', '.btn-selecionar-disciplina-destino', function () {
                        ...
                        {                            
                            if (isConfirm)
                            {
                                ...
                                jxhr = $.ajax({
                                    type: 'POST',
                                    url: '../Page/PlanoEnsinoMedicina.aspx/CopiarPlanoEnsino',
                                    data: data,
                                    contentType: 'application/json; charset=utf-8',
                                    dataType: 'json'
                                })
...
function carregarPlanoEnsinoTopicos() {
    ...
    jqxhr = $.ajax({
        type: 'POST',
        url: '../Page/PlanoEnsinoMedicina.aspx/TrazerConteudoTopico',
        data: JSON.stringify({
            idDisciplinaOfertaProfessor: idDisciplinaOfertaProfessor,
            idDisciplinaOferta: idDisciplinaOferta
        }),
...
function carregarPlanoEnsinoCronograma() {
    ...
    jqxhr = $.ajax({
        type: 'POST',
        url: '../Page/PlanoEnsinoMedicina.aspx/TrazerSemanasCronograma',
        data: JSON.stringify({
            idDisciplinaOfertaProfessor: idDisciplinaOfertaProfessor,
            idDisciplinaOferta: idDisciplinaOferta
        }),
...
function Autenticar(getIdCampus) {
    try {
        chamadaAjax("/View/Page/PlanoEnsinoMedicina.aspx", "UsuarioFuncionalidade", { idCampus: getIdCampus },
         function (objJson) {
