...
{
    public partial class RelatorioPeriodoMultasPagas : CommonPage
    {
        protected new void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                try
                {
                    ...
                    if (Audit.Autenticar("RF022"))
                    {
                        report.Load(Server.MapPath(@"~\View\Report\Biblioteca\Stimull\RelatorioPeriodoMultasPagas" + (ModeloRelatorio.Equals("2") ? "-Bruto.mrt" : ".mrt")));
                        ...
                        long TipoFiltroConsulta = Request.GetInt("TipoFiltroConsulta");
                        ...
                        // Quitado
                        if (TipoFiltroConsulta == 1)
                            query += " AND IIF(MultaSituacao.IdMultaSituacao = 1 OR ISNULL(NSM.TituloEmAberto,0) = 1, 0, 1) = 1 ";

                        // Pendente
                        if (TipoFiltroConsulta == 2)
                            query += " AND IIF(MultaSituacao.IdMultaSituacao = 1 OR ISNULL(NSM.TituloEmAberto,0) = 1, 0, 1) = 0 ";
                        ...
                        if (TipoFiltroConsulta == 1)
                            lstFitro.Add("Somente Títulos Quitados");

                        if (TipoFiltroConsulta == 2)
                            lstFitro.Add("Somente Títulos Pendentes");
