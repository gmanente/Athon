...
angular.module('ngFabForm').run(['$templateCache', function ($templateCache) {
    ...
    $templateCache.put('default-validation-msgs.html',
        "   <div class=\"validation\" ng-show=\"attrs.required==''|| attrs.required\"><ul class=\"list-unstyled validation-errors\" ng-messages=\"field.$error\" ng-show=\"field.$invalid && (field.$touched || field.$dirty || form.$triedSubmit)\"><li ng-message=\"required\">Este campo é obrigatório</li><li ng-message=\"ngFabEnsureExpression\">Por favor certifique-se que a informação digitada segue o padrão da condição do campo</li><li ng-message=\"password\">Por favor digite uma senha válida</li><li ng-message=\"email\">Por favor digite um endereço de email válido</li><li ng-message=\"pattern\">Formato do texto inválido para este campo</li><li ng-message=\"date\">Por favor digite uma data válida</li><li ng-message=\"time\">Por favor digite uma hora válida</li><li ng-message=\"datetime\">Por favor digite uma data e hora válida</li><li ng-message=\"datetime-local\">Por favor digite uma data e hora válida</li><li ng-message=\"number\">Por favor digite um número válido</li><li ng-message=\"color\">Por favor informe um código de cor válido</li><li ng-message=\"range\">O valor informado não esta dentro do intervalo permitido - {{attrs.min}} até {{attrs.max}}</li><li ng-message=\"textOnly\">Por favor certifique-se que a informação digitada contém apenas letras e acentos</li><li ng-message=\"month\">Por favor digite um Mês válido</li><li ng-message=\"url\">Por favor digite uma URL válida no formato http(s)://www.google.com</li><li ng-message=\"file\">Arquivo inválido</li><li ng-message=\"cpfincomplete\">O CPF Informado esta incompleto</li><li ng-message=\"cpfinvalid\">O CPF Informado é inválido</li><li ng-message=\"minlength\">Por favor digite pelo menos {{ attrs.minlength }} caracteres</li><li ng-message=\"maxlength\">Você digitou mais do que o máximo de {{ attrs.maxlength }} caracteres</li><li ng-message=\"ngFabMatch\">As {{ attrs.type ==='password'? 'senhas' : 'senhas' }} devem ser iguais</li><li ng-if=\"attrs.type == 'time' \" ng-message=\"min\">A hora informada deve ser de no mínimo {{ attrs.min | date: 'HH:MM' }}</li><li ng-message=\"max\" ng-if=\"attrs.type == 'time' \">A hora informada deve ser de no máximo {{attrs.max |date: 'HH:MM'}}</li><li ng-message=\"min\" ng-if=\"attrs.type == 'date' \">A data informada deve ser de no mínimo {{attrs.min | date:'YYYY-MM-DD'}}</li><li ng-message=\"max\" ng-if=\"attrs.type == 'date' \">A data informada deve ser de no máximo {{attrs.max | date: 'dd.MM.yy'}}</li><li ng-message=\"min\" ng-if=\"attrs.type === 'number' \">O número informado deve ser de no mínimo {{attrs.min}}</li><li ng-message=\"max\" ng-if=\"attrs.type === 'number' \">O número informado deve ser de no máximo {{attrs.max}}</li></ul><div class=\"validation-success\" ng-show=\"field.$valid && !field.$invalid\"></div></div>"
    );
